...
program
    .description(
        "For TD Playground core validation you can call the Playground \n" +
    ...
    .addOption(
        new commander.Option("-t, --type <type>", "The type of JSON documents that are passed as inputs")
            .choices(["TD", "TM", "AUTO"])
            .default("TD")
    )
    .option(
        "-i, --input <pathToInputs...>",
        "The files or the folders containing the files, which will be processed and/or validated. Multiple inputs can be provided.",
        undefined
    )
    .option(
        "-j, --no-jsonld",
        "Turn off the JSON-LD validation (for example because internet connection is not available)"
    )
    .option(
        "-d, --no-defaults",
        "Turn off the Full JSON Schema validation, which checks e.g. for default values being explicitly set"
    )
    .option("--no-tm-conformance", "Turn off TD to TM conformance checks (applicable when TD links to TM)")
    .option(
        "-a, --assertions",
        "Call the assertion report instead of the core validation, \n" +
            "if files with .csv ending are given as input merging assertion reports is done"
    )
    .option(
        "-o, --assertion-out <pathToOutput>",
...
function tdAssertionReport(inputParam) {
    ...
    doneEventEmitter.on("start", (tdName) => {
        bar.increment(1, { tdName });
    });

    bar.start(numberOfFilesAssertion, 0);
    assertTd(tdsToCheck, assertType, tdsToMerge, manualAssertions, doneEventEmitter).then(() => {
        bar.stop();
...
function mergeReports(reports) {
    return new Promise((res, rej) => {
        if (reports.length > 1) {
            assertMergeResults(reports).then(
                (merged) => {
        ...
        if (fs.existsSync(validPath) && fs.lstatSync(validPath).isDirectory()) {
            ...
            validNames.forEach((el) => {
                if (el.endsWith(".json") || el.endsWith(".jsonld")) {
                    ...
                    const thisProm = tdValidator(
                        tdToCheck,
                        console.log,
                        { checkDefaults: false, checkTmConformance: false },
                        suite
                    ).then(
                        (result) => {
        ...
        if (fs.existsSync(invalidPath) && fs.lstatSync(invalidPath).isDirectory()) {
            ...
            invalidNames.forEach((el) => {
                if (el.endsWith(".json") || el.endsWith(".jsonld")) {
                    ...
                    const suite = builder.testSuite().name(el);
                    const thisProm = tdValidator(tdToCheck, console.log, { checkDefaults: false }, suite).then(
                        (result) => {
                            if (statResult("failed", result.report)) {
                                invalidCount++;
                            } else {
            ...
            Promise.all(checkPromises).then(
                () => {
                    if (invalidNames.length === 0) {
                        ...
                        console.log(
                            "\nInvalidity test NOT successful, ",
                            invalidCount,
                            "/",
                            invalidNames.length,
                            "passed the invalidity test"
                        );
                    }
                },
                (err) => {
        ...
        if (fs.existsSync(warnPath) && fs.lstatSync(warnPath).isDirectory()) {
            ...
            warnNames.forEach((el) => {
                if (el.endsWith(".json") || el.endsWith(".jsonld")) {
                    ...
                    const suite = builder.testSuite().name(el);
                    const thisProm = tdValidator(tdToCheck, console.log, { checkDefaults: true }, suite).then(
                        (result) => {
                            if (statResult("failed", result.report)) {
            ...
            Promise.all(checkPromises).then(
                () => {
                    if (warnNames.length === 0) {
                        ...
                        console.log(
                            "\nWarning test NOT successful, ",
                            warnCount,
                            "/",
                            warnNames.length,
                            "passed the warning test"
                        );
                    }
                },
                (err) => {
...
function extractName(pathLike) {
    ...
    if (pathLike.indexOf(".") !== -1) {
        pathLike = pathLike.split(".").slice(0, -1).join(".");
    }
    return pathLike;
}
...
function tmAssertionReport(inputParam) {
    ...
    const bar = new cliProgress.SingleBar(
        { format: "progress [{bar}] {percentage}% | TM Name: {tmName} | {value}/{total} \n" },
        cliProgress.Presets.shades_classic
    );
    ...
    if (typeof inputParam === "object") {
        ...
        fs.readdirSync(inputParam).forEach((el) => {
            if (el.endsWith(".json") || el.endsWith(".jsonld")) {
                ...
                tmsToMerge.push(assertManualToJson(fs.readFileSync(path.join(inputParam, el), "utf-8")));
                numberOfFilesMerge++;
            } else {
                console.log("CANNOT HANDLE file in dir: ", path.join(inputParam, el));
            }
    ...
    const doneEventEmitter = new EventEmitter();
    doneEventEmitter.on("start", (tmName) => {
        bar.increment(1, { tmName });
    });

    bar.start(numberOfFilesAssertion, 0);
    assertTm(tmsToCheck, assertType, tmsToMerge, manualAssertions, doneEventEmitter).then(() => {
        setTimeout(() => {
            bar.stop();
        }, 100);
        ...
        if (fs.existsSync(validPath) && fs.lstatSync(validPath).isDirectory()) {
            ...
            validNames.forEach((el) => {
                if (el.endsWith(".json") || el.endsWith(".jsonld")) {
                    ...
                    const suite = builder.testSuite().name(el);
                    const thisProm = tmValidator(tmToCheck, console.log, { checkDefaults: false }, suite).then(
                        (result) => {
                            if (statResult("failed", result.report)) {
                                console.log(el, "was supposed to be valid but gave error");
                            } else if (statResult("warning", result.report)) {
        ...
        if (fs.existsSync(warnPath) && fs.lstatSync(warnPath).isDirectory()) {
            ...
            warnNames.forEach((el) => {
                if (el.endsWith(".json") || el.endsWith(".jsonld")) {
                    ...
                    const suite = builder.testSuite().name(el);
                    const thisProm = tmValidator(tmToCheck, console.log, { checkDefaults: true }, suite).then(
                        (result) => {
                            if (statResult("failed", result.report)) {
                                console.log(el, "was supposed to give a warning but gave error");
                                // result.console.forEach( line => {console.log(line)} )
...
function checkTm(tm, suite) {
    tmValidator(tm, console.log, { checkDefaults: myArguments.defaults, checkJsonLd: myArguments.jsonld }, suite).then(
        (result) => {
            console.log("OKAY \n");
            console.log("\n");
            console.log("--- Report ", JSON.parse(td).title, "---\n", result, "\n--------------");
