  ...
  describe("mapSecurity unit tests", () => {
    describe("mapSecurityString", () => {
      test("oauth2", () => {
        ...
        const computed = mapSecurityString(
          ["oauth2_sc"],
          oauth2Aap.securitySchemes,
          { oauth2_sc: ["limited"] }
        );
        const computed2 = mapSecurityString(
          "oauth2_sc",
          oauth2Aap.securitySchemes,
          { oauth2_sc: ["limited"] }
        );
  ...
  describe("mapSecurity integration tests", () => {
    describe("mapFormSecurity", () => {
      ...
      test("oauth2 single scope no security", () => {
        expect(
          mapFormSecurity(oauth2Definitions, undefined, ["limited"])
        ).toEqual(result);
        expect(mapFormSecurity(oauth2Definitions, undefined, "limited")).toEqual(
          result
        );
      });
      test("oauth2 single scope with security", () => {
        ...
        expect(mapFormSecurity(oauth2Definitions, security, ["limited"])).toEqual(
          result
        );
        expect(mapFormSecurity(oauth2Definitions, security, "limited")).toEqual(
          result
